...
export const InteractiveTempCalculator: React.FC = () => {
  ...
  return (
    <div className="my-6 rounded-2xl border border-stone-300 bg-stone-50/80 p-5 sm:p-6 shadow-sm">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <span className="text-xs text-stone-500 self-center">常見高山預設：</span>
        {presets.map((p) => (
          <button
            key={p.label}
            type="button"
            onClick={() => setTargetAlt(p.alt)}
            className="text-xs px-2.5 py-1 rounded bg-white hover:bg-emerald-50 text-stone-700 hover:text-emerald-800 border border-stone-200 transition-colors"
          >
            {p.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        ...
        <div className="bg-white p-3.5 rounded-xl border border-stone-200">
          <div className="flex justify-between items-center text-xs font-semibold text-stone-700 mb-2">
            ...
            <span className="font-mono text-emerald-700 font-bold">{baseTemp} °C</span>
          </div>
          <input
            type="range"
            min="10"
        ...
        <div className="bg-white p-3.5 rounded-xl border border-stone-200">
          <div className="flex justify-between items-center text-xs font-semibold text-stone-700 mb-2">
            <span className="flex items-center space-x-1">
              <Mountain className="w-3.5 h-3.5 text-sky-500" aria-hidden="true" />
              <span>目標海拔高度</span>
            </span>
            <span className="font-mono text-emerald-700 font-bold">{targetAlt} m</span>
          </div>
          <input
            type="range"
            min="500"
        ...
        <div className="bg-white p-3.5 rounded-xl border border-stone-200">
          <div className="flex justify-between items-center text-xs font-semibold text-stone-700 mb-2">
            <span className="flex items-center space-x-1">
              <Wind className="w-3.5 h-3.5 text-teal-500" aria-hidden="true" />
              <span>稜線風速</span>
            </span>
            <span className="font-mono text-emerald-700 font-bold">{windSpeed} m/s</span>
          </div>
          <input
            type="range"
            min="0"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 mb-3">
        ...
        <div className="bg-stone-900 text-white p-4 rounded-xl shadow-sm">
      ...
      <div className="flex items-start space-x-2 text-xs text-amber-800 bg-amber-50 p-3 rounded-lg border border-amber-200">
        <AlertCircle className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" aria-hidden="true" />
        <div>
          <span className="font-bold">概念示意，實際氣溫依當日氣象條件變化。</span>
